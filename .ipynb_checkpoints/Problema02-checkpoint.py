...
# 1. Crear una columna para el continente basado en el país
...
# 2. Crear una columna para el precio en USD si el precio es mayor que 100
df_wine['precio_categoria'] = df_wine['precio'].apply(lambda x: 'Alto' if x > 100 else 'Bajo')

# 3. Crear una columna de reseñas de vinos (cantidad de reviews)
df_wine['reseñas'] = df_wine['puntos'].apply(lambda x: 1 if x >= 90 else 0)

# Generar reportes
# 1. Vinos mejor puntuados por continente
report1 = df_wine.groupby('continente')[['puntos', 'precio']].max().reset_index()

# 2. Promedio de precio de vino y cantidad de reseñas obtenidas según país
...
sender_password = open('token.txt').read().strip()  # Reemplaza con tu token
